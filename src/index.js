// 捕获错误用
export const invokeWithErrorCatch = ({ fn = noop, errorHandler = noop, log = true }) => {
    try {
      return fn()
    } catch (err) {
      log && logError(err)
  ...
  export const wrapPromiseWithCache = fn => {
    return function wrappedPromise(...args) {
      return invokeWithErrorCatch({ fn: () => fn.call(this, ...args).catch(err => logError(err)) })
    }
  }
  ...
  export const stringify = val => invokeWithErrorCatch({ fn: () => JSON.stringify(val) })
  export const parse = val => invokeWithErrorCatch({ fn: () => JSON.parse(val) })
